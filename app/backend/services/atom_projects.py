...
from models.atom_projects import Atom_projects

logger = logging.getLogger(__name__)


# ------------------ Service Layer ------------------
class Atom_projectsService:
    """Service layer for Atom_projects operations"""
    ...
    async def create(self, data: Dict[str, Any], user_id: Optional[str] = None) -> Optional[Atom_projects]:
        """Create a new atom_projects"""
        try:
            if user_id:
                data['user_id'] = user_id
            obj = Atom_projects(**data)
            self.db.add(obj)
            await self.db.commit()
            await self.db.refresh(obj)
            logger.info(f"Created atom_projects with id: {obj.id}")
            return obj
        except Exception as e:
            await self.db.rollback()
            logger.error(f"Error creating atom_projects: {str(e)}")
            raise
    ...
    async def check_ownership(self, obj_id: int, user_id: str) -> bool:
        ...
        try:
            ...
            logger.error(f"Error checking ownership for atom_projects {obj_id}: {str(e)}")
            return False

    async def get_by_id(self, obj_id: int, user_id: Optional[str] = None) -> Optional[Atom_projects]:
        """Get atom_projects by ID (user can only see their own records)"""
        try:
            query = select(Atom_projects).where(Atom_projects.id == obj_id)
            if user_id:
                query = query.where(Atom_projects.user_id == user_id)
            result = await self.db.execute(query)
            return result.scalar_one_or_none()
        except Exception as e:
            logger.error(f"Error fetching atom_projects {obj_id}: {str(e)}")
            raise
    ...
    async def get_list(
        ...
        """Get paginated list of atom_projectss (user can only see their own records)"""
        try:
            ...
            conditions = []
            if user_id:
                conditions.append(Atom_projects.user_id == user_id)

            if query_dict:
                for field, value in query_dict.items():
                    if hasattr(Atom_projects, field):
                        column = getattr(Atom_projects, field)
            ...
            stmt = select(Atom_projects, func.count().over().label("total_count"))
            ...
            if sort:
                if sort.startswith('-'):
                    field_name = sort[1:]
                    if hasattr(Atom_projects, field_name):
                        stmt = stmt.order_by(getattr(Atom_projects, field_name).desc())
                else:
                    if hasattr(Atom_projects, sort):
                        stmt = stmt.order_by(getattr(Atom_projects, sort))
            else:
                stmt = stmt.order_by(Atom_projects.id.desc())
            ...
            if rows:
                ...
                items = []
                count_stmt = select(func.count()).select_from(Atom_projects)
                for condition in conditions:
            ...
            logger.error(f"Error fetching atom_projects list: {str(e)}")
            raise

    async def update(self, obj_id: int, update_data: Dict[str, Any], user_id: Optional[str] = None) -> Optional[Atom_projects]:
        """Update atom_projects (requires ownership)"""
        try:
            obj = await self.get_by_id(obj_id, user_id=user_id)
            if not obj:
                logger.warning(f"Atom_projects {obj_id} not found for update")
                return None
            ...
            await self.db.commit()
            await self.db.refresh(obj)
            logger.info(f"Updated atom_projects {obj_id}")
            return obj
        except Exception as e:
            await self.db.rollback()
            logger.error(f"Error updating atom_projects {obj_id}: {str(e)}")
            raise

    async def delete(self, obj_id: int, user_id: Optional[str] = None) -> bool:
        """Delete atom_projects (requires ownership)"""
        try:
            obj = await self.get_by_id(obj_id, user_id=user_id)
            if not obj:
                logger.warning(f"Atom_projects {obj_id} not found for deletion")
                return False
            await self.db.delete(obj)
            await self.db.commit()
            logger.info(f"Deleted atom_projects {obj_id}")
            return True
        except Exception as e:
            await self.db.rollback()
            logger.error(f"Error deleting atom_projects {obj_id}: {str(e)}")
            raise

    async def get_by_field(self, field_name: str, field_value: Any) -> Optional[Atom_projects]:
        """Get atom_projects by any field"""
        try:
            if not hasattr(Atom_projects, field_name):
                raise ValueError(f"Field {field_name} does not exist on Atom_projects")
            column = getattr(Atom_projects, field_name)
            field_value = self._coerce_field_value(column, field_value, field_name)
            result = await self.db.execute(
                select(Atom_projects).where(column == field_value)
            )
            return result.scalar_one_or_none()
        except Exception as e:
            logger.error(f"Error fetching atom_projects by {field_name}: {str(e)}")
            raise

    async def list_by_field(
        self, field_name: str, field_value: Any, skip: int = 0, limit: int = 20
    ) -> List[Atom_projects]:
        """Get list of atom_projectss filtered by field"""
        try:
            if not hasattr(Atom_projects, field_name):
                raise ValueError(f"Field {field_name} does not exist on Atom_projects")
            column = getattr(Atom_projects, field_name)
            field_value = self._coerce_field_value(column, field_value, field_name)
            result = await self.db.execute(
                select(Atom_projects)
                .where(column == field_value)
                .offset(skip)
                .limit(limit)
                .order_by(Atom_projects.id.desc())
            )
            return result.scalars().all()
        except Exception as e:
            logger.error(f"Error fetching atom_projectss by {field_name}: {str(e)}")
            raise